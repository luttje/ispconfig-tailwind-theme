ISPConfig.loadResponsiveMenu = function () {
    ...
    responsiveNavigation.innerHTML = '<div class="self-end items-center cursor-pointer mb-2 px-4 py-2 lg:hidden rounded hover:bg-gray-800 hover:text-white dark:bg-slate-500 dark:text-slate-100 dark:hover:bg-slate-400" @click="sidebarOpen = !sidebarOpen">&#10005;</div>';
    ...
    mainNavigation.querySelectorAll('a').forEach(function(item) {
        ...
        let activeClass = isActive ? 'active flex flex-row gap-2 items-center px-4 py-2 rounded bg-gray-800 text-white dark:bg-slate-400 dark:text-slate-100' : 'flex flex-row gap-2 items-center px-4 py-2 rounded hover:bg-gray-800 hover:text-white dark:hover:bg-slate-600 dark:hover:text-slate-100';
    ...
    subNavigation.querySelectorAll('a').forEach(function(item) {
        ...
        let classes = item.classList.contains('subnav-header') ? 'subnav-header text-gray-500 font-bold text-sm' : '';

        let newElement = document.createElement('li');
        newElement.innerHTML = '<a href="' + item.getAttribute('href') + '" ' + additionalAttribute + ' class="' + classes + ' flex flex-row items-center px-4 py-2 rounded hover:bg-gray-800 hover:text-white dark:hover:bg-slate-600 dark:hover:text-slate-100" @click="sidebarOpen = false">' + item.textContent + '</a>';
